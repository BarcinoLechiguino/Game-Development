...
bool j1App::LoadGame()
{
	...
	pugi::xml_document saveFile; //added 
	pugi::xml_node saveNode; //added
	pugi::xml_parse_result result = saveFile.load_file("saveFile.xml");

	if (ret == NULL)
	{
		LOG("Could not load saveFile.xml", result.description());
	}

	else
	{
		saveNode = saveFile.child("save");
		...
		while (item != NULL && ret == true)
		{
			item->data->load(saveFile.child(item->data->name.GetString()));
			item = item->next;
		}
	}
}
