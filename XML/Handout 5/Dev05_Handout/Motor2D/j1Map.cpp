	...
	for (int y = 0; y < data.height; y++) {
		for (int x = 0; x < data.width; x++) {
			...
			App->render->Blit(data.tilesets[0]->texture,
				...
				tileset->GetTileRect(gid_list[i]));
			i++;
...
{
	bool ret = true;
	p2SString tmp("%s%s", folder.GetString(), file_name); //carreguem el map

	pugi::xml_parse_result result = map_file.load_file(tmp.GetString()); //tmp.GetString() added
