...
void j1Map::Path(int x, int y)
{
	...
	iPoint current = goal;						//Sets the starting point of the backtracking to the goal tile's position.
	path.Pop(current);							//Pops current from the queue.

	while (current != visited.start->data)		//While the data members of current are different from the first element in the visited list.
	{
		int i = visited.find(current);			//.find gets the index number in the list of the given current tile.
		current = breadcrumbs[i];				//Assigns current the data members of the element at the given index. 
		path.PushBack(current);					//Adds current to the path list.
	}
	path.PushBack(visited.start->data);			//When the path is done and the current tile is the first one, then that first tile is added to path.
}
