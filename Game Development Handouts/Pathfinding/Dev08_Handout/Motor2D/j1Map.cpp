...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Pop(current) !=  NULL)
	{
		...
		neighbours[0] = { current.x + 1, current.y + 0}; //Right
		neighbours[1] = { current.x - 1, current.y + 0 }; //Left
		neighbours[2] = { current.x + 0, current.y + 1 }; //Up
...
bool j1Map::IsWalkable(int x, int y) const
{
	...
	if (x >= 0 && x < data.width && y >= 0 && y < data.height)		//Checks that the position received as an argument is within the map's dimensions. Calculated in tiles, not pixels.
	{
		int tile_id = data.layers.start->next->data->Get(x, y);		//Accesses the navigation layer (second layer) and gets the tile_id of each navigation layer tile.

		if (tile_id == 0)											//Tile_id = 0 means that that tile is walkable.
		{
			ret = true;
		}
		else
