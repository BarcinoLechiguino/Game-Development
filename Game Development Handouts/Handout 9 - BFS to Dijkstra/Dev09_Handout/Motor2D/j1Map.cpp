...
void j1Map::Path(int x, int y)
{
	...
	if (MovementCost(current.x, current.y) >= 0)	//If movement cost is 0 or higher (the value of out of bounds tiles is -1, not really but treated like that)
	{
		while (current != visited.start->data)		//While current's position data members are different from the first item in the visited list.
		{
			int i = visited.find(current);			//i is asigned the index number of current in the visited list.
			current = breadcrumbs[i];				//Assigns current the position data members of it's parent node. It's where current came from / The tile to which current is a neighbour and came from.
			path.PushBack(current);					//Allocates memory in the path dynArray for the current tile.
		}
	}
	path.PushBack(visited.start->data);				//Allocate memory in the array for the origin tile.
}
...
void j1Map::PropagateDijkstra()
{
	...
	if (frontier.Count() != 0)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if ((new_cost < cost_so_far[neighbour[i].x][neighbour[i].y] || visited.find(neighbour[i]) == -1) && MovementCost(neighbour[i].x, neighbour[i].y) >= 0)
			{
