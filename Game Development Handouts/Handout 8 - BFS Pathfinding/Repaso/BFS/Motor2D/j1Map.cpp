...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Count() != 0)
	{
		...
		neighbour[0] = { current.x + 1, current.y + 0 };
		neighbour[1] = { current.x + 0, current.y + 1 };
		neighbour[2] = { current.x - 1, current.y + 0 };
		neighbour[3] = { current.x + 0, current.y - 1 };

		// TODO 2: For each neighbor, if not visited, add it
		// to the frontier queue and visited list		
		for (int i = 0; i < 4; i++)
		{
			if (IsWalkable(neighbour[i].x, neighbour[i].y) == true)
			{
