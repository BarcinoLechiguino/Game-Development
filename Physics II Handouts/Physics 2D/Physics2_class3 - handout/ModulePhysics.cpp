...
bool PhysBody::Contains(int x, int y) const
{
	...
	bool hit = body->GetFixtureList()->GetShape()->TestPoint(body->GetTransform(), point);
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2RayCastInput input; //Its the incoming ray
	...
	b2RayCastOutput output; //The returned ray after input has reached the shape.

	bool hit = body->GetFixtureList()->GetShape()->RayCast(&output, input, body->GetTransform(), childIndex);
	if (hit)
	{
		normal_x = output.normal.x; //Returning ray posX
		normal_y = output.normal.y; //Returning ray posY
